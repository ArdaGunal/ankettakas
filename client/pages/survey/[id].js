...
export default function SurveyDetail() {
  ...
  const handleClaimPoint = async () => {
    try {
        if(!token) { 
          setStatus('completed'); 
          toast('Misafir modundasın. Puan kazanmak için giriş yapmalısın.', { icon: '👻' });
          return; 
        }
        const res = await axios.post(`${API_URL}/click/${survey._id}`, {}, { 
  ...
  const handleSendComment = async (e) => {
      ...
      try {
          const res = await axios.post(
            `${API_URL}/surveys/${survey._id}/comment`, 
            { text: commentText }, 
            { headers: { 'x-auth-token': token } }
          );
  ...
  const handleRateSubmit = async () => {
      ...
      try {
          const res = await axios.post(
            `${API_URL}/surveys/${survey._id}/rate`, 
            { stars: ratingStars }, 
            { headers: { 'x-auth-token': token } }
          );
  ...
  const handleSendReply = async (e, index) => {
      ...
      try {
          const res = await axios.post(
            `${API_URL}/surveys/${survey._id}/reviews/${index}/reply`, 
            { text: replyText }, 
            { headers: { 'x-auth-token': token } }
          );
  ...
  const handleDeleteComment = async (index) => {
      ...
      try {
          const res = await axios.delete(
            `${API_URL}/surveys/${survey._id}/reviews/${index}`, 
            { headers: { 'x-auth-token': token } }
          );
  ...
  // OY KONTROLÜ: Bu kullanıcı bu anketi oyladı mı?
  const hasRated = survey?.ratings?.some(r => r.username === loggedInUser);
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-10">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-300">
        ...
        <div className="bg-indigo-900 p-8 text-white text-center relative">
            <button 
              onClick={() => router.push('/')} 
              className="absolute top-4 left-4 bg-indigo-800 hover:bg-indigo-700 px-3 py-1 rounded text-sm border border-indigo-600"
            >
              ← Geri Dön
            </button>
            
            <div className="flex justify-center items-center gap-2 mb-3">
                <span className="bg-indigo-700 text-xs px-2 py-1 rounded-full uppercase tracking-wide border border-indigo-500">
        ...
        <div className="p-8 text-center border-b border-gray-200">
            ...
            <div className="max-w-md mx-auto">
                {status === 'idle' && (
                  <button 
                    onClick={handleStart} 
                    className="w-full py-4 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition shadow-lg text-lg transform hover:scale-105"
                  >
                    🚀 Ankete Git ve Başla
                  </button>
                )}
                ...
                {status === 'waiting' && (
                  <button 
                    disabled 
                    className="w-full py-4 rounded-xl font-bold text-gray-600 bg-gray-200 border border-gray-300 cursor-wait text-lg"
                  >
                    ⏳ Sekme Açık, Dönüş Bekleniyor...
                  </button>
                )}
                ...
                {status === 'ready' && (
                  <button 
                    ...
                    className="w-full py-4 rounded-xl font-bold text-white bg-green-600 hover:bg-green-700 animate-bounce shadow-lg text-lg"
                  >
                    ✅ Doldurdum, Puanı Ver!
                  </button>
                )}
                ...
                {status === 'completed' && (
                  <button 
                    disabled 
                    className="w-full py-4 rounded-xl font-bold text-green-800 bg-green-100 border border-green-300 text-lg"
                  >
                    ✓ İşlem Tamamlandı
                  </button>
                )}
            ...
            {!token && (
              <p className="text-xs text-red-500 font-bold mt-3">
                Puan kazanmak ve yorum yapmak için giriş yapmalısın.
              </p>
            )}
        </div>
        ...
        <div className="p-8 bg-gray-50">
            <h3 className="text-xl font-bold text-gray-900 mb-6 border-b pb-2 border-gray-300">
              💬 Yorumlar ve Puanlama
            </h3>

            {/* OYLAMA KISMI - SADECE OYLAMAYANLARA GÖSTER */}
            {token && !hasRated && (
                 <div className="bg-white p-4 rounded-xl shadow-sm border border-indigo-200 mb-6">
                     <p className="font-bold text-gray-800 mb-3 text-center">
                       ⭐ Bu Anketi Değerlendir
                     </p>
                     
                     <div className="flex items-center justify-center gap-3">
                        <select 
                          className="border border-gray-400 rounded-lg p-2 bg-white text-black font-medium" 
                          value={ratingStars} 
                        ...
                        <button 
                          ...
                          className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 transition text-sm shadow-md"
                        >
                            Oyla
                        </button>
                     </div>
            ...
            {/* OYLAMIŞ KULLANICILAR İÇİN GÜNCELLEME */}
            {token && hasRated && (
                 <div className="bg-white p-4 rounded-xl shadow-sm border border-green-200 mb-6">
                     ...
                     {cooldownTime > 0 ? (
                         <div className="text-center">
                             <div className="text-sm font-bold text-red-600 bg-red-100 px-4 py-2 rounded-lg inline-block">
                               ⏳ Oyu güncellemek için {formatTime(cooldownTime)} bekle
                             </div>
                         </div>
                     ) : (
                        <div className="flex items-center justify-center gap-3">
            ...
            {token && (
                 <form 
                   ...
                   className="bg-white p-4 rounded-xl shadow-sm border border-gray-300 mb-8"
                 >
                     <input 
                       type="text" 
                       placeholder="Görüşlerini yaz..." 
                       required 
                       className="w-full border border-gray-400 rounded-lg p-2 mb-2 text-black placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 outline-none bg-white" 
                       value={commentText} 
                       onChange={e => setCommentText(e.target.value)} 
                     />
                     <button className="bg-gray-800 text-white px-6 py-2 rounded-lg font-bold hover:bg-black transition text-sm shadow-md">
                       Yorumu Gönder
                     </button>
                 </form>
            )}
            ...
            <div className="space-y-4">
                {survey.comments && survey.comments.length > 0 ? (
                        ...
                        <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-gray-300">
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex gap-2 text-xs font-bold">
                                    {token && (
                                      <button 
                                        onClick={() => setReplyingToIndex(replyingToIndex === i ? null : i)} 
                            ...
                            {replyingToIndex === i && (
                                <form 
                                  ...
                                  className="mt-3 flex gap-2 pl-4 border-l-4 border-indigo-200"
                                >
                                    <input 
                                      autoFocus 
                                      type="text" 
                                      placeholder="Yanıtın..." 
                                      className="flex-1 border border-gray-400 rounded p-2 text-sm text-black bg-white" 
                                      value={replyText} 
                                      onChange={e => setReplyText(e.target.value)} 
                                    />
                                    <button className="bg-indigo-600 text-white px-3 py-1 rounded text-sm font-bold">
                                      Yolla
                                    </button>
                                </form>
                            )}
                            ...
                            {rev.replies && rev.replies.length > 0 && (
                                <div className="mt-3 space-y-2 pl-4 border-l-4 border-gray-200">
                                    {rev.replies.map((reply, rIndex) => (
                                        <div 
                                          key={rIndex} 
                                          className="bg-gray-100 p-2 rounded text-sm text-gray-800 border border-gray-200"
                                        >
                    ...
                    <div className="text-center text-gray-500 font-medium py-4 bg-white rounded-xl">
