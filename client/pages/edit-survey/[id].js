...
export default function SurveyDetail() {
  ...
  const [survey, setSurvey] = useState(null); // <-- Değişkenimiz bu
  ...
  useEffect(() => {
    ...
    const fetchSurvey = async () => {
        try {
            // DÜZELTME: Burada 'headers' YOK. Misafirler de veriyi çekebilir.
            const res = await axios.get(`${API_URL}/surveys/${id}`);
            setSurvey(res.data); // <-- setSurvey kullanıyoruz
        } catch (err) {
  ...
  // ... (Geri kalan kodlar, zamanlayıcı, yorumlar vb. aynen kalacak) ...
